...
export default function McpHubPage() {
  ...
  useEffect(() => {
    async function loadData() {
      try {
        setLoading(true);
        const [statusRes, toolsRes, accountsRes] = await Promise.all([
  ...
  return (
    <div className="max-w-6xl mx-auto space-y-8 animate-fadeIn">
          ...
          <div className="bg-white p-6 rounded-2xl border border-slate-200 shadow-xs space-y-5">
            ...
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pt-2">
              ...
              <button
                ...
                className="px-5 py-2.5 rounded-xl text-xs font-bold bg-indigo-600 hover:bg-indigo-700 disabled:opacity-50 text-white shadow-xs flex items-center justify-center gap-2 transition"
              >
                {generating ? (
                  <>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4">
            {CLIENT_OPTIONS.map((c) => (
              <button
                ...
                className={`p-4 rounded-xl text-left border transition relative space-y-1.5 ${
                  selectedClient === c.id
            ...
            <div className="bg-white p-6 rounded-2xl border border-slate-200 shadow-xs space-y-4">
              <div className="flex items-center justify-between border-b border-slate-100 pb-3">
                ...
                <button
                  ...
                  className="px-3.5 py-1.5 rounded-lg text-xs font-semibold bg-slate-100 hover:bg-slate-200 text-slate-800 flex items-center gap-1.5 transition"
                >
      ...
      {/* TAB 4: Interactive Test Console */}
      {activeTab === 'console' && (
        <div className="bg-white p-6 rounded-2xl border border-slate-200 shadow-xs space-y-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div className="space-y-4">
              ...
              <button
                ...
                disabled={executing}
                className="w-full py-2.5 rounded-xl text-xs font-bold bg-blue-600 hover:bg-blue-700 disabled:opacity-50 text-white flex items-center justify-center gap-2 transition shadow-xs"
              >
                {executing ? (
                  <>
                    <span className="w-3.5 h-3.5 border-2 border-white/30 border-t-white rounded-full animate-spin" />
